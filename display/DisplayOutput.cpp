/////////////////////////////////////////////////
// Serial port interface program to write text on the youBot display
/////////////////////////////////////////////////
...
int main(int argc, char* argv[]) {
  try {
    if (argc != 4) {
      std::cout << "invalid arguments \n ./displayoutput 'serial port' 'line2 text' 'line3 text' \n./displayoutput /dev/ttyACM0 'Hello World' 'Line 3'" << std::endl;
      return 0;
    }
    int fd = open_port(argv[1]);
    configure_port(fd);

    std::string line2text(argv[2]);
    std::string line3text(argv[3]);

    setText(fd, line2, line2text);
    setText(fd, line3, line3text);
